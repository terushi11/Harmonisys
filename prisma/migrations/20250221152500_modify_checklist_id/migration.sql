/*
  Warnings:

  - The primary key for the `Checklist` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - You are about to drop the column `id` on the `Checklist` table. All the data in the column will be lost.

*/
-- AlterTable
ALTER TABLE "Checklist" DROP CONSTRAINT "Checklist_pkey",
DROP COLUMN "id",
ADD CONSTRAINT "Checklist_pkey" PRIMARY KEY ("category", "key", "unahonId");
