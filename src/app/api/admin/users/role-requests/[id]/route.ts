...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const requestRow = await prisma.roleChangeRequest.findUnique({
    where: { id },
    select: { id: true, userId: true, toRole: true, status: true },
  });
  ...
  if (requestRow.status !== RequestStatus.PENDING) {
    return NextResponse.json({ success: false, message: "Request is not pending." }, { status: 409 });
  }

  if (requestRow.toRole !== UserType.RESPONDER && requestRow.toRole !== UserType.ADMIN) {
    return NextResponse.json({ success: false, message: "Invalid requested role." }, { status: 400 });
  }
