...
const Bar3D = (props: any) => {
  ...
  // gradient shades (slightly lighter on top, normal at bottom)
  ...
  // unique id per bar (stable enough)
  ...
  return (
    <g>
      ...
      {/* thin highlight edge */}
      <rect x={fx + 1} y={fy + 1} width={Math.max(0, fw - 2)} height={Math.max(0, fh - 2)} fill="transparent" stroke="rgba(255,255,255,0.22)" strokeWidth="1" />
    </g>
  );
};
...
// force BarChart re-mount to replay animation on re-enter
const [barChartKey, setBarChartKey] = useState(0);


  useEffect(() => {
  // ✅ Admin always fetches (no gating)
  if (!canSeeAnalytics && userRole !== 'ADMIN') return;

  const fetchEvents = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/irs/events');
      const result = await response.json();
      setEvents(Array.isArray(result?.data) ? (result.data as UIEvent[]) : []);
    } catch (error) {
  ...
  const locObs = makeObserver(
    () => {
      setLocInView(true);
      setBarChartKey((k) => k + 1); // ✅ replay chart anim every time it enters
    },
    () => setLocInView(false)
  );
  ...
  const locationData = useMemo(() => {
    ...
    events.forEach((e) => {
      const rawLocation = (e as any).location;

      const location =
        typeof rawLocation === 'string'
          ? rawLocation
          : rawLocation?.locationName
          ? String(rawLocation.locationName)
          : rawLocation?.locationCode
          ? String(rawLocation.locationCode)
          : rawLocation?.locationDescription
          ? String(rawLocation.locationDescription)
          : 'Unknown';
  ...
  // ✅ colored mini-cards (soft tint per status)
  const statCardBase =
  ...
  },
  {
    ...
    // ✅ label helper (trim long location names)
    ...
    return (
      <div className="rounded-2xl border border-white/60 bg-white/90 px-4 py-2 shadow-xl">
        ...
        <p className="text-xs text-slate-700">
          <span className="font-extrabold">{payload[0].value}</span> events
        </p>
      </div>
    );
  };
...
>
  ...
  >
    See Incidents
        ...
        {/* About + Core Features (STANDARD only) */}
      ...
      <CardBody className="pt-3 space-y-5 px-5 sm:px-7 pb-7">
        ...
        <div className="rounded-2xl border border-white/55 bg-white/50 p-5 shadow-sm">
          <div className="border-l-4 border-[#7B122F] pl-4 text-slate-700 leading-relaxed text-[15px] sm:text-[16px] [text-align:justify]">
            ...
            NICHE Centers in the Regions for R&amp;D (NICER) program for the Disaster Risk Reduction and Management
            in Health Center of the University of the Philippines Manila, headed by Dr. Carlos Primero D. Gundran.
          </div>
        </div>
      </CardBody>
    </Card>
    ...
    >
      ...
      <CardBody className="pt-3 space-y-4 px-5 sm:px-7 pb-7">
        ...
        <div id="reports-analytics" className="mt-10 pb-8 scroll-mt-20">
          {/* ✅ Event Status Overview (outside cards) */}
          <div className="mb-8" ref={statsRef}>
            ...
            {/* Ongoing */}
            <Card className={`${statCardBase} bg-gradient-to-br from-blue-500/15 via-blue-500/10 to-blue-500/5 to-white/60 border border-blue-500/20`}>
              <CardBody className="p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[14px] sm:text-[14.5px] font-extrabold text-slate-900 leading-tight">
                        Ongoing Events
                      </p>
                      <p className="text-[12.5px] text-black">Currently active</p>
                    </div>
                  </div>

                  <div className="text-right flex flex-col items-end">
                    <span className="text-3xl font-black text-blue-600 leading-none">
                      {loading ? '—' : eventStats.ongoing}
                    </span>
                    <span className="text-[12px] text-black mt-1 leading-none">
                      events
                    </span>
                  </div>
                </div>
            ...
            style={{
              width: statsInView
                ? `${eventStats.total ? Math.round((eventStats.ongoing / eventStats.total) * 100) : 0}%`
                : '0%',
          ...
          />
            ...
            {/* Completed */}
            <Card className={`${statCardBase} bg-gradient-to-br from-green-500/15 via-green-500/10 to-green-500/5 to-white/60 border border-green-500/20`}>
              <CardBody className="p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[12.5px] text-black">Successfully resolved</p>
                    </div>
                  </div>

                  <div className="text-right flex flex-col items-end">
                    <span className="text-3xl font-black text-green-600 leading-none">
                      {loading ? '—' : eventStats.completed}
                    </span>
                    <span className="text-[12px] text-black mt-1 leading-none">
                      events
                    </span>
            ...
            style={{
              width: statsInView
                ? `${eventStats.total ? Math.round((eventStats.completed / eventStats.total) * 100) : 0}%`
                : '0%',
              transition: 'width 2000ms cubic-bezier(0.16, 1, 0.3, 1)',
                ...
                />
                  ...
                  {/* Pending */}
                  <Card className={`${statCardBase} bg-gradient-to-br from-amber-500/15 via-amber-500/10 to-amber-500/5 to-white/60 border border-amber-500/20`}>
                    <CardBody className="p-6">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="h-11 w-11 rounded-2xl bg-amber-500/25 flex items-center justify-center shadow-md">
                          ...
                          <div>
                            ...
                            <p className="text-[12.5px] text-black">Awaiting response</p>
                          </div>
                        </div>

                        <div className="text-right flex flex-col items-end">
                          <span className="text-3xl font-black text-amber-600 leading-none">
                            {loading ? '—' : eventStats.pending}
                          </span>
                          <span className="text-[12px] text-black mt-1 leading-none">
                            events
                          </span>
                      ...
                      <div className="mt-4 h-2 w-full rounded-full bg-amber-500/15 overflow-hidden">
                        <div
                  className="h-full rounded-full bg-amber-500"
                  style={{
                    width: statsInView
                      ? `${eventStats.total ? Math.round((eventStats.pending / eventStats.total) * 100) : 0}%`
                      : '0%',
                  transition: 'width 2000ms cubic-bezier(0.16, 1, 0.3, 1)',
              ...
              />
                ...
                {/* Total */}
                <Card className={`${statCardBase} bg-gradient-to-br from-[#7B122F]/15 via-[#7B122F]/10 to-[#7B122F]/5 to-white/60 border border-[#7B122F]/20`}>
                  <CardBody className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="h-11 w-11 rounded-2xl bg-[#7B122F]/25 flex items-center justify-center shadow-md">
                          <ClipboardList className="w-6 h-6 text-[#7B122F]" />
                        </div>
                        <div>
                          <p className="text-[14px] sm:text-[14.5px] font-extrabold text-slate-900 leading-tight">
                            Total Events
                          </p>
                          <p className="text-[12.5px] text-black">All recorded</p>
                        </div>
                      </div>

                      <div className="text-right flex flex-col items-end">
                        <span className="text-3xl font-black text-[#7B122F] leading-none">
                          {loading ? '—' : eventStats.total}
                        </span>
                        <span className="text-[12px] text-black mt-1 leading-none">
                          events
                        </span>
          ...
          {/* ✅ Events by Location title OUTSIDE (same style as above) */}
          <div className="mt-6" ref={locRef}>
            ...
            <Card className={`${cardGlass} bg-gradient-to-br from-[#7B122F]/10 via-white/55 to-white/35`}>
              {/* soft glow layers (like your top 4 stat cards vibe) */}
              ...
              <CardBody className="pt-5 px-4 sm:px-6 pb-5">
                  ...
                  <div className="lg:col-span-1 rounded-3xl border border-white/55 bg-white/55 px-6 py-5">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-2xl font-black text-slate-900">{loading ? '—' : eventStats.total}</p>
                      </div>
                    </div>
                              ...
                              <div className="mt-2 h-2.5 w-full rounded-full bg-slate-200 overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    ...
                                    width: locInView
                                      ? `${eventStats.total ? Math.round((loc.value / eventStats.total) * 100) : 0}%`
                                      : '0%',
                                    transition: 'width 2000ms cubic-bezier(0.16, 1, 0.3, 1)',
                ...
                />
                  ...
                  {/* ✅ "3D-like" colored bars: shadow + rounded + per-bar color */}
                  ...
                  {open && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                      <Card className="bg-white/85 backdrop-blur-md shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-white/60 rounded-3xl">
                        ...
                        <CardBody className="p-6">
                          <Questionnaire onClose={handleCloseModal} openSuccessModal={() => setShowSuccessDialog(true)} />
                        </CardBody>
                      </Card>
                    </div>
                  )}
